...
export default function SupportPage() {
  ...
  const remaining = used === null ? null : DAILY_LIMIT - used
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (!res.ok) {
      ...
      if (res.status === 429) setUsed(DAILY_LIMIT)
      return
    }
    setUsed(prev => (prev ?? 0) + 1)
    setSent(true)
  }
        ...
        <Card>
          <CardContent className="py-12 text-center space-y-3">
            ...
            {remaining !== null && remaining > 0 && (
              <p className="text-xs text-zinc-400">วันนี้ส่งได้อีก {remaining} ครั้ง</p>
            )}
            {remaining !== null && remaining <= 0 && (
              <p className="text-xs text-amber-600">ใช้ครบ {DAILY_LIMIT} ครั้งแล้ววันนี้</p>
            )}
            {remaining !== null && remaining > 0 && (
    ...
    <div className="p-5 md:p-8 max-w-2xl">
      ...
      {limitReached ? (
        <Card>
          <CardContent className="py-12 text-center space-y-3">
            <div className="text-5xl">⏳</div>
            <p className="text-lg font-medium text-zinc-800">ใช้ครบ {DAILY_LIMIT} ครั้งแล้ววันนี้</p>
            <p className="text-sm text-zinc-500">กรุณาลองใหม่ในวันพรุ่งนี้</p>
            <p className="text-xs text-zinc-400">หากเร่งด่วน ติดต่อ: pressinth@gmail.com</p>
          </CardContent>
        </Card>
      ) : (
          ...
          <div className="mt-4 flex items-center justify-between">
            <p className="text-xs text-zinc-400">
              {remaining === null
                ...
                : `ส่งได้อีก ${remaining}/${DAILY_LIMIT} ครั้งวันนี้`}
            </p>
